...
void hardwareMonitor()
{
  ...
  if (httpCode > 0) 
  {
    Serial.printf("JSON available HTTP code %d\n", httpCode);
    ...
    Serial.printf("Filter memory usage %d\n", filter.memoryUsage());
    DynamicJsonDocument hwm(10000);
    DeserializationError error = deserializeJson(hwm, http.getStream(), DeserializationOption::Filter(filter), DeserializationOption::NestingLimit(12));
    if (error) 
    {
        String errorStr = error.c_str();
        Serial.print ("Hardware parsing ERROR: ");
    ...
    else
    {
      ...
      hwm.shrinkToFit();
      Serial.printf("HWM memory usage %d\n", hwm.memoryUsage());
  ...
  {
    Serial.println("ERROR http.GET() == 0");
    //Зажигаем красный светодиод при ошибке
  }
  http.end();
