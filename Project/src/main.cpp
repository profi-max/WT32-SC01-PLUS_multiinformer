...
//Переменные конфигурации дисплея
  const uint16_t screenWidth  = SCREEN_WIDTH; //ширина экрана
  const uint16_t screenHeight = SCREEN_HEIGHT; //высота экрана
  //Подсветка дисплея
...
//    static lv_color_t buf[screenWidth * screenHeight / 8];
    lv_obj_t * wifistatus; //статус wifi
    lv_obj_t * ui_status_clock; //Часы в статус баре
    lv_obj_t * tabview; //Вкладки
  
    //Экраны
    //1 экран
    lv_obj_t * statusbox;//служебные сообщения на 1 вкладке
    lv_obj_t * displayclock;//часы
    lv_obj_t * calendar;//календарь
    lv_obj_t * ui_weatherimage_main;//Изображение погоды
    lv_obj_t * roomtemp;//температура с датчика
    lv_obj_t * roompress;//давление с датчика
    lv_obj_t * roomhumid;//влажность с датчика
    lv_obj_t * roomair;// показания датчика качества воздуха
    lv_obj_t * roomair_bar;//индикатор качества воздуха
    lv_obj_t * roomair_bar_label;//баллы качества воздуха
    //4 экран курсы валют
    lv_obj_t * cur_table;//таблица
    lv_obj_t  * ui_cur_title; //Заголовок таблицы
    //2 экран. Монитор ПК
    lv_obj_t * cpumeter; //загрузка и теипература процессора
    lv_meter_indicator_t * cpu_indic;//полоса загрузки процессора
    lv_meter_indicator_t * cpufan_indic;//вентилятор процессора
    lv_meter_indicator_t * cputemp_indic;//полоса температуры процессора
    lv_obj_t * gpumeter;//загрузка и температура видеокарты
    lv_meter_indicator_t * gpu_indic;//загрузка видеокарты
    lv_meter_indicator_t  *gputemp_indic;//температура видеокарты
    lv_meter_indicator_t * gpufan_indic;//вентилятор видеокарты
    lv_obj_t * rambar;//полоса оперативной памяти
    lv_obj_t * gpurambar;//полоса памяти видеокарты
    lv_obj_t * ui_upload_label;//передача по сети
    lv_obj_t * ui_download_label;//прием по сети
    ...
    lv_anim_t ramgpu;
    //Различные надписи
    lv_obj_t * cpu_load_label;
    lv_obj_t * cpu_temp_label;
    lv_obj_t * cpu_fan_label;
    lv_obj_t * gpu_load_label;
    lv_obj_t * gpu_temp_label;
    lv_obj_t * gpu_fan_label;
    lv_obj_t * ram_val_label;
    lv_obj_t * gpuram_val_label;
    //3 экран. Объявление переменных экрана погоды
    lv_obj_t * windmeter; //Отображение направления ветра
    lv_obj_t * ui_windspeed;// надпись для скорости ветра
    lv_obj_t * ui_windgust;// надпись для скорости ветра
    lv_meter_indicator_t * wind_indic;//индикатор направления ветра
    //Различные надписи для экрана погоды
    lv_obj_t * weathercity;//Название города
    lv_obj_t *ui_weathertemp;//температура
    lv_obj_t *ui_humid;//влажность
    lv_obj_t *ui_feeltemp;//температура ощущется как
     lv_obj_t *ui_avgtemp;//максимальная и минимальная температуры
    lv_obj_t *ui_weatherdesc;//описание погодных условий
    lv_obj_t *ui_pressure;//атмосферное давление
    lv_obj_t *ui_visiblity;//видимость
    lv_obj_t *ui_sunset;//закат
    lv_obj_t *ui_sunrise;//рассвет
    lv_obj_t *ui_weatherimage;//картинка с погодными условиями
    //графики
    lv_obj_t *ui_pressurechart;//график давления
    lv_obj_t *ui_tempchart;//график температуры
    lv_obj_t *ui_humidchart;//график влажности
    //массивы
    lv_chart_series_t * ui_prser; //массив координат графика давления
    lv_chart_series_t * ui_tempser; //массив координат графика температуры
    lv_chart_series_t * ui_huser; //массив координат графика влажности
    //Экран радио
    lv_obj_t * radio_volumembar; //полоса громкости
    lv_obj_t * radio_station_num_label; //номер станции
    lv_obj_t * radio_playing_value_label; //текущий трек
    lv_obj_t  * radio_playing_label; //название трека
    lv_obj_t  * radio_visualiser; //визуализация трека
    lv_chart_series_t  * radio_visualiser_series; //массив данных для визуализации
    lv_obj_t * playlistwin; //редактор плейлиста
    lv_obj_t * playlist_table;//таблица плейлиста
    lv_obj_t * radio_play_btn;

    //7 экран настройки
    lv_obj_t * slider_label;
    lv_obj_t * set_tabview; //Вкладки настроек
    lv_obj_t * kb; //клавиатура
    lv_obj_t * wt_ta; //поле ввода API погоды
    lv_obj_t * pc_ta; //поле ввода адреса пк
    lv_obj_t * ya_ta; //поле ввода API яндекса
    lv_obj_t * wtl_ta; //поле ввода местоположения погоды
    lv_obj_t * yandex_int_slider_label;
    lv_obj_t * weather_int_slider_label;
    lv_obj_t * pc_int_slider_label;
    lv_obj_t * bme_int_slider_label;
    lv_obj_t * gmt_slider_label;
    lv_obj_t * wifitable;
    lv_obj_t * wifipass_ta;
    lv_obj_t * wifissid_ta;
    lv_obj_t * slider_daytime_label;
...
//-------------------------------------------------------------------------------------------
  ...
  ESPxWebFlMgr filemgr(filemanagerport); //  порт
  //Таймеры
...
static const char * my_year_list = {
    "2040\n2039\n2038\n2037\n2036\n2035\n2034\n2033\n2032\n2031\n2030\n2029\n2028\n2027\n2026\n2025\n2024\n2023\n2022\n2021\n"
    "2020\n2019\n2018\n2017\n2016\n2015\n2014\n2013\n2012\n2011\n2010\n2009\n2008\n2007\n2006\n2005\n2004\n2003\n2002\n2001\n"
    "2000\n1999\n1998\n1997\n1996\n1995\n1994\n1993\n1992\n1991\n1990\n1989\n1988\n1987\n1986\n1985\n1984\n1983\n1982\n1981\n"
};
...
//Создаем экранные объекты
 static lv_style_t bigtext;
    ...
    lv_style_set_text_font(&bigtext, &ui_font_fira24);       
...
//Первый экран
  ...
  lv_calendar_header_dropdown_set_year_list(calendar, my_year_list);
  ...
  lv_obj_set_style_text_font(roomtemp, &ui_font_fira24, LV_PART_MAIN| LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(roomhumid, &ui_font_fira24, LV_PART_MAIN| LV_STATE_DEFAULT);
  ...
  lv_obj_set_style_text_font(roompress, &ui_font_fira24, LV_PART_MAIN| LV_STATE_DEFAULT);
...
//Вклака 2. параметры ПК
  ...
  lv_obj_set_style_text_font(ui_cpuname, &ui_font_fira24, 0);
  ...
  lv_obj_set_style_text_font(ui_gpuname, &ui_font_fira24, 0);
...
//Вкладка 3 экран погоды
  ...
  lv_obj_set_style_text_font(ui_weathertemp, &ui_font_fira24, LV_PART_MAIN| LV_STATE_DEFAULT);
  //Влажность
  ui_humid = lv_label_create(tab3);
  lv_obj_set_pos(ui_humid, 250, 30);
  lv_label_set_text(ui_humid,"55%");
  lv_obj_set_style_text_font(ui_humid, &ui_font_fira24, LV_PART_MAIN| LV_STATE_DEFAULT);
